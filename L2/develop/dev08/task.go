...
func main() {
	...
	for scanner.Scan() {
		...
		for _, value := range comands {
			...
			case "pwd": //показать путь до текущего каталога
				fmt.Println(dir)
			case "echo": //вывод аргумента в STDOUT
				fmt.Println(strings.TrimSpace(strings.TrimPrefix(value, "echo")))
			case "kill": //"убить" процесс, переданный в качестве аргумента
			...
			case "ps": //выводит общую информацию по запущенным процессам
