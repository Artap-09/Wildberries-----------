...
func main() {
	...
	for i := 1; i < len(os.Args)-1; i++ {
		...
		case "-i": //из файла
			i++
			if i > len(os.Args)-1 {
				log.Fatalln("укажите название файла со списком")
			}

			urls = os.Args[i]
		case "-P": // в указанный каталог
			i++
			if i >= len(os.Args) {
				log.Fatalln("укажите путь куда сохранять файлы")
			}

			nameFile = os.Args[i]
		case "-A": // скачать весь сайт
		case "-c": // продолжить загрузку
			i++
			if i >= len(os.Args)-1 {
				log.Fatalln("укажите название файла со списком")
			}

			nameFile = path.Join(os.Args[i], nameFile)
			contin = true
		case "-o": // название файла
			i++
			if i >= len(os.Args)-1 {
				log.Fatalln("укажите название файла со списком")
			}
